...
def generate_sonata_files_from_memodel(memodel_path, output_path, mtype, threshold_current, holding_current):
    """
    ...
    logging.info(f"SONATA single cell circuit created at {output_path}")
...
def create_nodes_file(hoc_file, morph_file, output_path, mtype, threshold_current, holding_current):
    ...
    with h5py.File(nodes_h5_path, 'w') as f:
        ...
        group_0.create_dataset('model_template', (1,), dtype=h5py.string_dtype())[0] = f"hoc:{Path(hoc_file).stem}"
        group_0.create_dataset('model_type', (1,), dtype='int32')[0] = 0
        group_0.create_dataset('morph_class', (1,), dtype='int32')[0] = 0
        group_0.create_dataset('morphology', (1,), dtype=h5py.string_dtype())[0] = f"morphologies/{Path(morph_file).stem}"
        group_0.create_dataset('mtype', (1,), dtype=h5py.string_dtype())[0] = mtype

        # Coordinates and rotation
        for name in ['x', 'y', 'z', 'rotation_angle_xaxis', 'rotation_angle_yaxis', 'rotation_angle_zaxis']:
            group_0.create_dataset(name, (1,), dtype='float32')[0] = 0.0

        # Quaternion orientation
        orientation = {'orientation_w': 1.0, 'orientation_x': 0.0, 'orientation_y': 0.0, 'orientation_z': 0.0}
        for name, value in orientation.items():
            group_0.create_dataset(name, (1,), dtype='float64')[0] = value

        # Optional fields
        group_0.create_dataset('morphology_producer', (1,), dtype=h5py.string_dtype())[0] = "biologic"
